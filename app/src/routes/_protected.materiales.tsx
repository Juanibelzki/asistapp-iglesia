...
export const Route = createFileRoute('/_protected/materiales')({
...
const STAGE_BADGE_CLASS: Record<TargetStage, string> = {
  todos: 'bg-slate-800 text-zinc-300 border-slate-700/60',
  'niño': 'bg-yellow-500/10 text-yellow-500 border-yellow-500/30',
  'adolescente': 'bg-slate-500/10 text-slate-300 border-slate-500/30',
  'adulto': 'bg-slate-500/10 text-slate-300 border-slate-500/30',
};
          ...
          !loadError && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {materials.map((item) => (
                <div
                  key={item.id}
                  className="bg-[#141C2B]/50 border border-slate-700/60 rounded-2xl p-5 flex flex-col gap-3 hover:border-slate-600/60 transition"
                >
                  ...
                  <div className="flex items-center gap-2 pt-2 border-t border-slate-800 mt-auto">
                    {item.file_url ? (
                      <a
                        ...
                        className="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-yellow-500 hover:bg-yellow-600 text-slate-950 font-bold text-xs transition shadow-lg shadow-yellow-500/20"
                      >
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full text-sm text-zinc-400 file:mr-3 file:px-3 file:py-2 file:rounded-lg file:border file:border-slate-700 file:bg-[#0B0F17] file:text-zinc-200 file:text-xs file:font-bold hover:file:bg-slate-800 transition"
                />
                {file && (
                  <p className="text-[11px] text-yellow-500 mt-1.5 truncate">✓ {file.name}</p>
                )}
              </div>
